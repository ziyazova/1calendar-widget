...
const PinCard = styled.div<{ $col?: string; $row?: string; $mobileCol?: string; $mobileRow?: string; $hideOnMobile?: boolean }>`
  border-radius: 16px;
  overflow: hidden;
  cursor: default;
...
/* Grid layout: 4 columns x 4 rows, every column fills exactly 4 rows */
const PIN_ITEMS: { type: 'image' | 'text'; image?: string; bg?: string; color?: string; title?: string; desc?: string; col?: string; row?: string; mobileCol?: string; mobileRow?: string; hideOnMobile?: boolean }[] = [
  ...
  /* col 4 */ { type: 'image', image: '/widget-calendar2.png', col: '4', row: '1 / 4', hideOnMobile: true },
  /* col 4 */ { type: 'text', bg: '#FFF8E0', title: 'Free forever', desc: 'No premium tier, no limits.', col: '4', row: '4 / 5', hideOnMobile: true },
];
...
export const PinterestGallery: React.FC = () => {
  return (
    <PinterestGrid>
      {PIN_ITEMS.map((pin, i) => (
        <PinCard key={i} $col={pin.col} $row={pin.row} $mobileCol={pin.mobileCol} $mobileRow={pin.mobileRow} $hideOnMobile={pin.hideOnMobile}>
          {pin.type === 'image' ? (
